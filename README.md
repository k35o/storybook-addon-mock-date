...
- **The mock is scoped to the story.** It is installed from a `beforeEach` hook — so a story's own `beforeEach` sees the mocked time — and removed when the story ends, so nothing leaks into the next story. To keep one story on the real clock under a preview-level `mockingDate`, use the new `mockingDate: { disable: true }`.
- **`advanceMockedTime` and `runAllMockedTimers` return a promise** that resolves once the UI has committed. Existing calls still advance the clock right away; add `await` to assert on the result without a hand-written flush.
- **Import the helpers and types from `storybook-addon-mock-date`.** The `/preview` entry now carries only the preview annotations (a `beforeEach` hook instead of `decorators`), so `advanceMockedTime`, `runAllMockedTimers`, `getMockedClock` and types such as `MockingDateParam` are no longer exported from it.
