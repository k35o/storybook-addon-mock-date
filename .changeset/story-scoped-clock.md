...
- The `/preview` entry exports `beforeEach` and `initialGlobals` instead of `decorators` and `initialGlobals`. Projects that spread the entry into a plain-object `preview.ts` or register it through `definePreview` are unaffected.
